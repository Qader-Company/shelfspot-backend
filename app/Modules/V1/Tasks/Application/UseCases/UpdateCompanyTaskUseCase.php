...
class UpdateCompanyTaskUseCase
{
    ...
    public function execute(Task $task, array $data, array $files = []): Task
    {
        return DB::transaction(function () use ($task, $data, $files) {
            /** @var Task $lockedTask */
            $lockedTask = $this->taskRepository->getByIdAndLockedForUpdate($task->id, ['services.products']);
            ...
            $lockedTask->services()->delete();
            $taskServiceModels = $this->createTaskServices($lockedTask, $taskServices);
